...
class AudioRecorder:
    ...
    def console_language(self, console):
        ...
        ser.write(f"Console language set to: {console}")

    def toggle_recording(self):
        self.recording = not self.recording
        if self.recording:
            ser.write(self.trans["recording"])
            self.frames = []  # Clear previous recording frames
            threading.Thread(target=self.record).start()
        else:
            ser.write(self.trans["stopped"])
    ...
    def save_audio(self):
        ...
        ser.write(self.trans["transcription"], transcription)
    ...
    def set_hotkey(self, hotkey):
        while True:
            ...
            if data == 'start':
                ...
                break
        ser.write(self.trans["tutorial"],  hotkey)
        ser.write(self.trans["halt"])
        keyboard.wait('alt+c')

    def set_language(self, language):
        self.language = language
        ser.write(self.trans["language"], self.language)

    def set_model(self, model):
        self.model = whisper.load_model(model)
        ser.write(self.trans["model"],  model)
